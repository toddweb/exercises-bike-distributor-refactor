...
namespace BikeDistributor.Test
{
	...
	public class ReceiptGeneratorTests
	{
		...
		private ReceiptGenerator sut = new ReceiptGenerator();
		...
		public void ReceiptOneDefy()
		{
			...
			Assert.AreEqual(ResultStatementOneDefy, sut.Generate(order, ReceiptFormats.Text));
		}
...
Total: $1,072.50";
		...
		public void ReceiptOneElite()
		{
			...
			order.ApplyTaxRate(TaxRate);
			Assert.AreEqual(ResultStatementOneElite, sut.Generate(order, ReceiptFormats.Text));
		}
...
Total: $2,145.00";
		...
		public void ReceiptOneDuraAce()
		{
			...
			order.ApplyTaxRate(TaxRate);
			Assert.AreEqual(ResultStatementOneDuraAce, sut.Generate(order, ReceiptFormats.Text));
		}
...
Total: $5,362.50";
		...
		public void HtmlReceiptOneDefy()
		{
			...
			order.AddLine(new Line(Defy, 1));
			order.ApplyTaxRate(TaxRate);
			Assert.AreEqual(HtmlResultStatementOneDefy, sut.Generate(order, ReceiptFormats.Html));
		}
		...
		public void HtmlReceiptOneElite()
		{
			...
			order.AddLine(new Line(Elite, 1));
			order.ApplyTaxRate(TaxRate);
			Assert.AreEqual(HtmlResultStatementOneElite, sut.Generate(order, ReceiptFormats.Html));
		}
		...
		public void HtmlReceiptOneDuraAce()
		{
			...
			order.AddLine(new Line(DuraAce, 1));
			order.ApplyTaxRate(TaxRate);
			Assert.AreEqual(HtmlResultStatementOneDuraAce, sut.Generate(order, ReceiptFormats.Html));
		}
		...
		public void XmlReceiptOneDefy()
		{
			...
			order.AddLine(new Line(Defy, 1));
			order.ApplyTaxRate(TaxRate);
			Assert.AreEqual(XmlResultStatementOneDefy, sut.Generate(order, ReceiptFormats.Xml));
		}
		...
		public void XmlReceiptOneElite()
		{
			...
			order.AddLine(new Line(Elite, 1));
			order.ApplyTaxRate(TaxRate);
			Assert.AreEqual(XmlResultStatementOneElite, sut.Generate(order, ReceiptFormats.Xml));
		}
		...
		public void XmlReceiptOneDuraAce()
		{
			...
			order.AddLine(new Line(DuraAce, 1));
			order.ApplyTaxRate(TaxRate);
			Assert.AreEqual(XmlResultStatementOneDuraAce, sut.Generate(order, ReceiptFormats.Xml));
		}
